...
namespace InMoment.Application.Tests.Groups.RemoveAdmin;

public sealed class RemoveAdminHandlerTests
{
    private readonly Mock<IGroupRepository> _groups = new();
    private readonly Mock<IUnitOfWork> _uow = new();
    private readonly Mock<ICurrentUser> _current = new();

    private RemoveAdminHandler Create()
    ...
    public async Task Handle_ShouldThrowNotFoundException_WhenGroupNotFound()
    {
        ...
        var act = () => handler.Handle(
            new RemoveAdminCommand(groupId, targetUserId),
    ...
    public async Task Handle_ShouldThrowForbiddenException_WhenActorIsNotOwner()
    {
        ...
        var act = () => handler.Handle(
            new RemoveAdminCommand(group.Id, memberId),
            CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_ShouldDemoteAdminToMember_WhenActorIsOwner()
    {
        var ownerId = Guid.NewGuid();
        var adminId = Guid.NewGuid();
        ...
        var result = await handler.Handle(
            new RemoveAdminCommand(group.Id, adminId),
            CancellationToken.None);

        result.Should().Be(Unit.Value);
        group.IsAdmin(adminId).Should().BeFalse();
        group.IsMember(adminId).Should().BeTrue();

        _uow.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_ShouldSaveChanges_WhenTargetAlreadyMember()
    {
        var ownerId = Guid.NewGuid();
        var memberId = Guid.NewGuid();
        ...
        group.IsMember(memberId).Should().BeTrue();
